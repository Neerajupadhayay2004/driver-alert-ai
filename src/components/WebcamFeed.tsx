...
import { 
  Camera, CameraOff, Loader2, AlertCircle, Zap, Eye, EyeOff, Activity,
  Shield, ShieldAlert, Maximize2, Radio, Scan, Target, AlertTriangle, Timer, Crosshair
} from 'lucide-react';
...
export function WebcamFeed({ onMetricsUpdate, onAlertLevelChange, onHistoryUpdate }: WebcamFeedProps) {
  ...
  const {
    isLoading: modelLoading, isModelLoaded, error: modelError,
    metrics, alertLevel, history, confidence, startDetection, stopDetection,
  } = useFaceDetection();
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      if (latencyRef.current.length > 0) {
        setDetectionLatency(Math.round(latencyRef.current.reduce((a, b) => a + b, 0) / latencyRef.current.length));
        latencyRef.current = [];
      }
  ...
  useEffect(() => { onMetricsUpdate?.(metrics); }, [metrics, onMetricsUpdate]);
  useEffect(() => { onAlertLevelChange?.(alertLevel); }, [alertLevel, onAlertLevelChange]);
  useEffect(() => { onHistoryUpdate?.(history); }, [history, onHistoryUpdate]);
  ...
  const handleToggle = async () => {
    if (isActive) {
      stopDetection(); stopWebcam(); setIsActive(false);
    } else {
      await startWebcam(); setIsActive(true);
    }
  };
  ...
  const statusText = useMemo(() => {
    if (modelLoading) return 'Loading AI...';
    if (error) return 'Error';
    if (!isModelLoaded) return 'Initializing...';
  ...
  return (
    <motion.div ref={containerRef} initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="relative">
      <div className={cn(
        ...
        <video ref={videoRef} className={cn('w-full h-full object-cover', !isStreaming && 'hidden')} playsInline muted />
        <canvas ref={canvasRef} className={cn('absolute inset-0 w-full h-full', !isStreaming && 'hidden')} />
          ...
          {isStreaming && (
            <>
              {['top-2 left-2 border-l-2 border-t-2', 'top-2 right-2 border-r-2 border-t-2',
                ...
                <motion.div key={pos} initial={{ opacity: 0, scale: 0.5 }} animate={{ opacity: 0.8, scale: 1 }} exit={{ opacity: 0 }}
                  transition={{ delay: i * 0.05 }}
                  className={cn('absolute w-6 h-6 sm:w-8 sm:h-8',
                    alertLevel === 'critical' || alertLevel === 'severe' ? 'border-critical' : 'border-primary', pos
                  )}
                />
              ))}
            </>
        ...
        <AnimatePresence>
          {isStreaming && metrics.faceDetected && (
            <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.8 }}
              ...
              {/* Scanning ring */}
              <motion.div
                className={cn("absolute w-44 h-56 sm:w-52 sm:h-64 rounded-full border border-dashed",
                  alertLevel === 'critical' ? 'border-critical/50' :
                  alertLevel === 'severe' ? 'border-critical/40' :
                  alertLevel === 'fatigued' || alertLevel === 'drowsy' ? 'border-warning/50' : 'border-primary/40'
                )}
                animate={{ rotate: 360 }} transition={{ duration: 8, repeat: Infinity, ease: "linear" }}
              />
              {/* Main box */}
              <motion.div
                className={cn("relative w-32 h-40 sm:w-40 sm:h-52 border-2 rounded-xl",
                  alertLevel === 'critical' ? 'border-critical' :
                  alertLevel === 'severe' ? 'border-critical/70' :
                  alertLevel === 'fatigued' || alertLevel === 'drowsy' ? 'border-warning' : 'border-primary'
                )}
                animate={{
                  boxShadow: alertLevel === 'critical' || alertLevel === 'severe'
                    ? ['0 0 15px rgba(255,68,68,0.4)', '0 0 30px rgba(255,68,68,0.6)', '0 0 15px rgba(255,68,68,0.4)']
                    : ['0 0 10px rgba(0,255,255,0.3)', '0 0 25px rgba(0,255,255,0.5)', '0 0 10px rgba(0,255,255,0.3)']
                }}
                transition={{ duration: 1.5, repeat: Infinity }}
              >
                <motion.div className="absolute top-1/2 left-0 right-0 h-px bg-gradient-to-r from-transparent via-primary/50 to-transparent"
                  animate={{ opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 2, repeat: Infinity }} />
                <motion.div className="absolute top-0 bottom-0 left-1/2 w-px bg-gradient-to-b from-transparent via-primary/50 to-transparent"
                  animate={{ opacity: [0.3, 0.7, 0.3] }} transition={{ duration: 2, repeat: Infinity, delay: 0.5 }} />
                <motion.div className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-2 h-2 rounded-full",
          ...
          <>
            ...
            <motion.div initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
              ...
              <div className="flex items-center gap-2 bg-background/90 backdrop-blur-sm px-2 py-1 rounded-lg border border-border/50">
                <motion.div className={cn('w-2 h-2 rounded-full', metrics.faceDetected ? 'bg-success' : 'bg-warning')}
              ...
              <AnimatePresence>
                {metrics.faceDetected && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                    ...
                    {metrics.eyesOpen ? <Eye className="w-3 h-3 text-success" /> : <EyeOff className="w-3 h-3 text-warning" />}
                    <span className={cn("text-[10px] sm:text-xs font-medium", metrics.eyesOpen ? "text-success" : "text-warning")}>
              ...
              <AnimatePresence>
                {metrics.faceDetected && (
                  <motion.div initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -5 }}
                    className="flex items-center gap-1.5 bg-background/90 backdrop-blur-sm px-2 py-1 rounded-lg border border-border/50">
                    <Crosshair className="w-3 h-3 text-primary" />
                    <span className="text-[10px] sm:text-xs font-mono">{confidence}%</span>
                    <div className="w-12 h-1 bg-secondary rounded-full overflow-hidden">
                      <motion.div className={cn("h-full rounded-full",
                        confidence >= 80 ? "bg-success" : confidence >= 50 ? "bg-warning" : "bg-critical"
                      )} animate={{ width: `${confidence}%` }} transition={{ duration: 0.3 }} />
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
            ...
            <motion.div initial={{ opacity: 0, x: 10 }} animate={{ opacity: 1, x: 0 }}
              ...
              {isActive && (
                <>
                  <div className={cn("flex items-center gap-1.5 backdrop-blur-sm px-2 py-1 rounded-lg border",
                    detectionLatency < 50 ? "bg-success/20 border-success/50" : detectionLatency < 100 ? "bg-warning/20 border-warning/50" : "bg-critical/20 border-critical/50"
                  )}>
                    <Timer className="w-3 h-3" />
                    <span className="text-[10px] sm:text-xs font-mono font-medium">{detectionLatency}ms</span>
                  </div>
                  <div className={cn("flex items-center gap-1.5 backdrop-blur-sm px-2 py-1 rounded-lg border",
                    fps >= 25 ? "bg-success/20 border-success/50" : fps >= 15 ? "bg-warning/20 border-warning/50" : "bg-critical/20 border-critical/50"
                  )}>
                    <Zap className={cn("w-3 h-3", fps >= 25 ? "text-success" : fps >= 15 ? "text-warning" : "text-critical")} />
                    <span className="text-[10px] sm:text-xs font-mono font-medium">{fps} FPS</span>
                  </div>
                  <motion.div className="flex items-center gap-1.5 bg-critical/90 px-2 py-1 rounded-lg"
                  ...
                  <Button variant="ghost" size="icon" className="w-7 h-7 bg-background/90 backdrop-blur-sm border border-border/50"
            ...
            {/* Bottom left - Connection status + head pose */}
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
              ...
              <div className="flex items-center gap-1.5 bg-background/90 backdrop-blur-sm px-2 py-1 rounded-lg border border-border/50">
                {metrics.faceDetected ? <Radio className="w-3 h-3 text-success" /> : <Scan className="w-3 h-3 text-warning animate-pulse" />}
                <span className={cn('text-[10px] sm:text-xs font-medium',
                  metrics.faceDetected ? 'text-success' : modelLoading ? 'text-primary' : 'text-muted-foreground'
                )}>{statusText}</span>
              </div>
              <AnimatePresence>
                {metrics.faceDetected && (
              ...
              {metrics.faceDetected && (
                <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 10 }}
                  ...
                  <div className="flex flex-col items-end gap-1.5">
                    <motion.div className={cn("flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] sm:text-xs font-mono font-bold",
                      metrics.perclos >= 50 ? "bg-critical/90 text-white" : metrics.perclos >= 25 ? "bg-warning/90 text-foreground" : "bg-success/90 text-white"
                    )} animate={metrics.perclos >= 50 ? { scale: [1, 1.05, 1] } : {}} transition={{ duration: 0.5, repeat: Infinity }}>
                      <Eye className="w-3 h-3" /><span>PERCLOS</span><span className="font-bold">{metrics.perclos.toFixed(1)}%</span>
                    </motion.div>
                    <div className="flex items-center gap-2">
                      <div className={cn("flex items-center gap-1 px-2 py-1 rounded-lg border text-[10px] sm:text-xs font-mono",
                        metrics.blinkRate < 8 || metrics.blinkRate > 25 ? "bg-warning/20 border-warning/50" : "bg-background/90 border-border/50"
                      )}>
                        <Activity className={cn("w-3 h-3", metrics.blinkRate < 8 || metrics.blinkRate > 25 ? "text-warning" : "text-primary")} />
                        <span>{metrics.blinkRate}</span><span className="text-muted-foreground">/min</span>
                      </div>
                      {metrics.yawnCount > 0 && (
                        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }}
                          ...
                          <AlertTriangle className="w-3 h-3 text-warning" /><span>{metrics.yawnCount} yawns</span>
                        </motion.div>
                      )}
                    </div>
                  </div>
            ...
            <AnimatePresence>
              {alertLevel !== 'alert' && metrics.faceDetected && (
                <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
                  className={cn("absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 px-4 py-2 rounded-xl backdrop-blur-md border-2",
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm sm:text-base font-bold uppercase tracking-wider">
                      {alertLevel === 'critical' ? '⚠️ CRITICAL' : alertLevel === 'severe' ? '⚠️ SEVERE' :
              ...
              {modelLoading ? (
                ...
                <motion.div key="ready" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                  ...
                  <div className="relative mb-4">
                    <motion.div className="w-20 h-20 sm:w-24 sm:h-24 rounded-full bg-gradient-to-br from-primary/20 to-primary/5 flex items-center justify-center border border-primary/30"
                    ...
                    <motion.div className="absolute inset-0 rounded-full border-2 border-primary/50"
                  ...
                  <div className="flex flex-wrap justify-center gap-2 mt-4">
                    {['Eye Tracking', 'Blink Detection', 'Yawn Detection', 'Head Pose', 'Auto-Calibration'].map((feature, i) => (
                      <motion.span key={feature} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
      ...
      <div className="absolute -bottom-5 sm:-bottom-6 left-1/2 -translate-x-1/2 flex items-center gap-3">
        <Button onClick={handleToggle} disabled={modelLoading || !!modelError}
          variant={isActive ? 'destructive' : 'default'} size="lg"
          className={cn('rounded-full w-12 h-12 sm:w-14 sm:h-14 p-0 shadow-lg transition-all',
            !isActive && !modelLoading && 'animate-glow-pulse hover:scale-110',
            isActive && 'hover:scale-95'
          )}>
          {modelLoading ? <Loader2 className="w-5 h-5 sm:w-6 sm:h-6 animate-spin" /> :
           isActive ? <CameraOff className="w-5 h-5 sm:w-6 sm:h-6" /> :
